  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="userlist" border stripe>
        ...
        <!--索引列-->
        <el-table-column label="Club Name" prop="C_name">
        </el-table-column>
        <el-table-column label="Founder ID" prop="Founder_id">
        </el-table-column>
        <el-table-column label="Found Time" prop="Found_time">
        </el-table-column>
    ...
    <el-dialog title="Add Club" :visible.sync="addDialogVisible" width="50%" @close="addDialogClosed">
      ...
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="95px">
        <el-form-item label="Club Name" prop="C_name">
          <el-input v-model="addForm.C_name"></el-input>
        </el-form-item>
        <el-form-item label="Founder Id" prop="Founder_id">
          <el-input v-model="addForm.Founder_id"></el-input>
        </el-form-item>
        <el-form-item label="Found Time" prop="Found_time">
          <el-input v-model="addForm.Found_time"></el-input>
        </el-form-item>
        <el-form-item label="Update Time" prop="Update_time">
    ...
    <el-dialog title="Edit Club" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
      <el-form :model="editForm" :rules="editFormRules" ref="editFormRef" label-width="70px">
        ...
        <el-form-item label="Founder Id" prop="Founder_id">
          <el-input v-model="editForm.Founder_id"></el-input>
        </el-form-item>
        <el-form-item label="Found Time" prop="Found_time">
...
<script>
  export default {
    ...
    methods: {
      async getUserList() {
        const { data: res } = await this.$http.get('clubs', { params: this.queryInfo })
        if (res.meta.status !== 200) return this.$message.error('获取列表失败')
        console.log(res)
        this.userlist = res.data.clubs
      ...
      addUser() {
        this.$refs.addFormRef.validate(async valid => {
          if (!valid) return
          // 可以发起添加用户的网络请求
          const { data: res } = await this.$http.post('clubs', this.addForm)
      ...
      async showEditDialog(C_id) {
        // console.log(M_id)
        const { data: res } = await this.$http.get('clubs/' + C_id)
      ...
      editUserInfo() {
        this.$refs.editFormRef.validate(async valid => {
          ...
          const { data: res } = await this.$http.put(
            'clubs/' + this.editForm.C_id,
            {
              C_name: this.editForm.C_name,
              Founder_id: this.editForm.Founder_id,
      ...
      async removeUserById(C_id) {
        ...
        const { data: res } = await this.$http.delete('clubs/' + C_id)
